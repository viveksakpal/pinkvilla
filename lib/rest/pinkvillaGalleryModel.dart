...
class PinkvillaGalleryModel {
  List<Nodes> nodes;

  PinkvillaGalleryModel({this.nodes});

  PinkvillaGalleryModel.fromJson(Map<String, dynamic> json) {
    if (json['nodes'] != null) {
  ...
  static List<Nodes> listFrmJson(List<dynamic> json) {
    return json == null ? new List<Nodes>() : json.map((
        value) => new Nodes.fromJson(value)).toList();
  }
...
class Node {
  ...
  Node(
      {this.title,
      @JsonKey(name:'nid_dont_use')
      this.nidDontUse,
      @JsonKey(name:'field_photo_image_section')
